                                  ...
                                  osrm::engine::api::ResultT &result) const
{
    ...
    auto request_mapping_src_to_dst = params.source_destination_mapping ==
                                      api::TableParameters::MappingType::OneToOne
                                          ? api::TableParameters::MappingType::OneToOne
                                          : api::TableParameters::MappingType::ManyToMany;
    
    if ((params.source_destination_mapping == api::TableParameters::MappingType::OneToOne && params.sources.size() <= 0) || params.destinations.size() <= 0)
    {
        return Error("SourceOrDestinationIndicesEmpty", "source and destination indices should not be empty for one to one mapping", result);
    }

    if (params.source_destination_mapping == api::TableParameters::MappingType::OneToOne && params.sources.size() != params.destinations.size() )
    {
        return Error("UnequalSourceAndDestinationIndices", "source and destination indices should be equal in number", result);
    }
    ...
    {     
         ...
         {
           auto one_to_one_table_result = algorithms.ManyToManySearch(snapped_phantoms, {params.sources[row]}, {params.destinations[row]}, request_distance);
           if( row < number_of_entries)
           {
                durations_table[row] = one_to_one_table_result.first[0];
                if (request_distance) {distances_table [row] = one_to_one_table_result.second[0];}
           }
         }
        ...
        params.scale_factor != 1)
    {
        for (std::size_t row = 0; row < num_sources; row++)
        {   
            auto start_index = request_mapping_src_to_dst == api::TableParameters::MappingType::OneToOne ? row : 0;
            auto end_index = request_mapping_src_to_dst == api::TableParameters::MappingType::OneToOne ? row + 1 : num_destinations;
            for (std::size_t column = start_index ; column < end_index; column++)
            {
                const auto &table_index = request_mapping_src_to_dst == api::TableParameters::MappingType::OneToOne ? row : row * num_destinations + column;
